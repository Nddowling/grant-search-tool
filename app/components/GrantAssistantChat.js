...
export default function GrantAssistantChat({ userProfile, onGrantSelect, onClose, isOpen, onSearchResults }) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-50 flex items-stretch p-4">
      {/* Backdrop - semi-transparent so you can see results behind */}
      <div
        className="fixed inset-0 bg-black/30"
        onClick={onClose}
      />

      {/* Modal - positioned on the right side, smaller width */}
      <div className="relative w-[380px] flex flex-col bg-slate-900 rounded-xl border border-slate-700 overflow-hidden shadow-2xl ml-auto">
        {/* Header - compact */}
        <div className="flex items-center justify-between px-3 py-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white">
          <div className="flex items-center gap-2">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
            </svg>
            <span className="font-semibold text-sm">Grant Assistant</span>
          </div>
          {onClose && (
            <button
              onClick={onClose}
            ...
            {message.type === 'status' ? (
              ...
              <div className="w-full">
                <div className="text-xs text-green-400 mb-2 flex items-center gap-1">
                  ...
                  Found {message.content.totalFound} grants - see results below
                </div>
              </div>
            ) : message.type === 'error' ? (
              <div className="max-w-[90%] p-2 rounded-lg bg-red-500/20 text-red-400 text-xs">
                {message.content}
              </div>
            ) : (
              <div
                className={`max-w-[90%] p-2 rounded-lg text-xs whitespace-pre-wrap ${
                  message.role === 'user'
      ...
      <div className="p-3 border-t border-slate-700">
        <div className="flex gap-2">
          <textarea
            ...
            rows={2}
            className="flex-1 bg-slate-800 border border-slate-600 rounded-lg px-3 py-2 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none text-xs"
            disabled={isLoading}
          />
          <button
            onClick={sendMessage}
            disabled={!input.trim() || isLoading}
            className="px-3 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-700 disabled:cursor-not-allowed text-white rounded-lg transition-colors flex items-center justify-center"
          >
            {isLoading ? (
